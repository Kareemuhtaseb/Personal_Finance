...
<template>
  ...
  <div class="lg:hidden fixed bottom-0 left-0 right-0 z-50 backdrop-blur-xl bg-white/10 border-t border-white/20 shadow-2xl shadow-blue-500/20 relative">
    ...
    <div class="absolute inset-0 bg-white/2"></div>
    
    <!-- Animated border gradient -->
    <div class="absolute inset-0 bg-blue-500/10 opacity-50"></div>
    <div class="absolute inset-[1px] bg-gray-900/95"></div>
      ...
      <router-link
        ...
        :class="[
          $route.path === item.href
            ? 'text-blue-300 bg-blue-500/20 border border-blue-400/30 backdrop-blur-sm shadow-lg shadow-blue-500/20'
            : 'text-white/80 hover:text-white hover:bg-white/10',
          `animate-delay-${Math.min(index * 100, 1000)}`
        ]"
      >
        <!-- Active state background -->
        <div v-if="$route.path === item.href" class="absolute inset-0 bg-blue-500/10 rounded-xl"></div>
    ...
    <div class="absolute -top-6 left-1/2 transform -translate-x-1/2">
      <router-link
        to="/expenses/new"
        class="w-14 h-14 bg-blue-500 rounded-full flex items-center justify-center shadow-2xl shadow-blue-500/30 hover:shadow-blue-500/50 transition-all duration-300 hover:scale-110 relative overflow-hidden group"
      >
        <!-- Button background animation -->
        <div class="absolute inset-0 bg-blue-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-full"></div>
        ...
        <!-- Glow effect -->
        <div class="absolute inset-0 rounded-full bg-blue-400 opacity-50 blur-md group-hover:opacity-75 transition-opacity duration-300"></div>
      </router-link>
    </div>
  </div>
</template>
